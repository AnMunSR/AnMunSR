<div align="center">

# Trần Thị Ánh Nguyệt 🌚

### DevOps • Cloud • Cybersecurity • Software Engineering

<p>
  <a href="mailto:tranthianhnguyet259@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>

  <a href="https://www.linkedin.com/in/tr%E1%BA%A7n-th%E1%BB%8B-%C3%A1nh-nguy%E1%BB%87t-735348283/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>

  <a href="https://github.com/AnMunSR">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

<img src="https://komarev.com/ghpvc/?username=AnMunSR&style=for-the-badge"/>

</div>

---

# 👩‍💻 About Me

Final-year Information Security student at Posts and Telecommunications Institute of Technology (PTIT HCM) with strong interests in:

- DevOps & Cloud Engineering
- Cybersecurity & Web Application Security
- DevSecOps & Infrastructure Automation
- Secure Software Development
- Cloud Infrastructure Deployment

Currently focusing on cloud deployment, CI/CD automation, infrastructure security, and secure software development practices.

---

# 🎓 Education

### Posts and Telecommunications Institute of Technology (PTIT HCM)

- Bachelor of Information Security
- 📍 Ho Chi Minh City, Vietnam
- 📅 2022 – 2027
- 📈 GPA: **3.46 / 4.0**

---

# 🛠 Tech Stack

### 💻 Programming Languages

<p>
  <img src="https://skillicons.dev/icons?i=java,cpp,python,javascript,php" />
</p>

---

### ☁️ Cloud • DevOps • Infrastructure

<p>
  <img src="https://skillicons.dev/icons?i=gcp,docker,kubernetes,terraform,githubactions,linux,bash,git" />
</p>

---

### 🔐 Cybersecurity & Networking

<p>
  <img src="https://img.shields.io/badge/OWASP%20ZAP-00549E?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Nmap-214478?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Wireshark-1679A7?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/SQLMap-CC2927?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/Kali%20Linux-557C94?style=for-the-badge&logo=kalilinux&logoColor=white"/>
</p>

---

### 🗄 Database

<p>
  <img src="https://skillicons.dev/icons?i=mysql" />
  <img src="https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white"/>
</p>

---

# 🚀 Experience & Technical Activities

## 🔐 Cybersecurity Research

- Researching and analyzing web vulnerabilities:
  - XSS
  - SQL Injection
  - WAF Bypass Techniques

- Simulating web attacks and penetration testing in lab environments

- Applying secure coding practices to web applications

- Writing vulnerability analysis reports and proposing security countermeasures

---

## ☁️ DevOps & Cloud

- Deploying infrastructure on Google Cloud Platform (GCP)

- Infrastructure as Code using Terraform

- Building CI/CD pipelines with GitHub Actions

- Working with Docker and Kubernetes environments

- Configuring Linux servers, networking, firewall rules, and load balancing

---

## 💻 Software Development

- Full-stack web application development

- Backend API implementation and authentication systems

- Database integration using MySQL and SQL Server

- Secure architecture implementation for modern web systems

---

# 📂 Featured Projects

## 🔹 Bypass WAF (Web Application Firewall)

### Security Research Project

- Simulated web attacks and XSS exploitation
- Researched WAF mechanisms and bypass techniques
- Practiced SQL Injection and XSS testing
- Wrote security analysis reports and countermeasures

### Technologies & Tools
`SQLMap` `OWASP ZAP` `Nmap` `ModSecurity` `Kali Linux`

---

## 🔹 Network Security Configuration on GCP

### DevOps & Cloud Project

- Designed and deployed cloud infrastructure on GCP
- Configured:
  - VPC
  - Firewall Rules
  - Internet Gateway
  - Web Servers
  - Load Balancer

- Automated deployment using Terraform and GitHub Actions

### Technologies & Tools
`GCP` `Terraform` `GitHub Actions` `Linux`

---

## 🔹 Laptop & Accessories E-commerce Website

### Full-stack Development Project

- Developed frontend and backend system
- Implemented:
  - Authentication system
  - User management
  - Database integration
  - MoMo payment gateway integration

### Technologies & Tools
`Next.js` `JavaScript` `SQL Server`

---

# 🤝 Open-source & Collaboration

## Public Projects
- Web security practice environments
- Cryptography implementations (AES, DES)
- Full-stack web applications
- Cloud & DevOps deployment projects

## Collaborative Projects
- Contributed to security-focused repositories
- Participated in cloud and Kubernetes deployment labs
- Worked on CI/CD and infrastructure automation projects

---

# 📊 GitHub Statistics

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=AnMunSR&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=AnMunSR&layout=compact&theme=tokyonight"/>

</div>

---

# 📈 Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=AnMunSR&theme=tokyo-night"/>

</div>

---

# 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=AnMunSR&theme=tokyonight"/>

</div>

---

# 🏆 Achievements

- Academic Excellence Scholarship (Years 1, 2, 3)
- Outstanding Youth Union Member (2025)
- Participated in ICPC Programming Contest (School Level)
- Active contributor to academic and technical projects

---

# 🎯 Career Objective

Seeking internship opportunities in:

- DevOps Engineering
- Cloud Engineering
- DevSecOps
- Cybersecurity Engineering

Long-term goal:

To become a DevSecOps Engineer specializing in secure cloud infrastructure, automation, and application security.

---

# 🤝 Connect With Me

<p align="center">
  <a href="mailto:tranthianhnguyet259@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>

  <a href="https://www.linkedin.com/in/tr%E1%BA%A7n-th%E1%BB%8B-%C3%A1nh-nguy%E1%BB%87t-735348283/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>

  <a href="https://github.com/AnMunSR">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

---

<div align="center">

### ⭐ Focused on continuous learning in DevOps, Cloud Computing, and Cybersecurity

</div>
